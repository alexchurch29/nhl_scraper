...
SELECT z.season, z.game_type, z.game_id, z.date, CASE WHEN r.team = z.home_team THEN 'Home' ELSE 'Away' END venue, 
    ...
    ifnull(faceoffwon.Faceoffs_Won,0) as Faceoffs_Won, ifnull(faceofflost.Faceoffs_Lost,0) as Faceoffs_Lost, 
    ifnull((round(ifnull(faceoffwon.Faceoffs_Won,0), 2) / (round(ifnull(faceoffwon.Faceoffs_Won,0), 2) + 
    round(ifnull(faceofflost.Faceoffs_Lost,0), 2))),0.0) as 'Faceoff%'
...
LEFT OUTER JOIN (SELECT p.game_id, p.name, p.team, r.pos, (p.time-1)/1200 + 1 as period, case when p.team = z.home_team 
    then s.home_strength || 'v' || s.away_strength else s.away_strength || 'v' || s.home_strength end strength, 
    round(count(p.name),2)/60 as TOI
...
ON missed.name = r.name and missed.pos = r.pos and missed.game_id = r.game_id and missed.strength = strength.strength 
and missed.period = strength.period
...
'''
SELECT z.season, z.game_type, z.game_id, z.date, CASE WHEN r.team = z.home_team THEN 'Home' ELSE 'Away' END venue, 
    strength.period, strength.strength, r.name, r.team, r.pos, 1 as GP, icetime.TOI, ifnull(cf.CF,0) as CF, 
    ifnull(ca.CA,0) as CA, ifnull(round(ifnull(cf.CF,0),2) / (round(ifnull(cf.CF,0),2) + round(ifnull(ca.CA,0),2)),0) 
    as 'CF%', ifnull(ff.FF,0) as FF, ifnull(fa.FA,0) as FA, ifnull(round(ifnull(ff.FF,0),2) / (round(ifnull(ff.FF,0),2) 
    ...
    as 'SF%', ifnull(gf.GF,0) as GF, ifnull(ga.GA,0) as GA, ifnull(round(ifnull(gf.GF,0),2) / (round(ifnull(gf.GF,0),2)
    + round(ifnull(ga.GA,0),2)),0) as 'GF%', ifnull(round(ifnull(gf.GF,0),2) / (round(ifnull(sf.SF,0),2)),0) 
    as 'On_Ice_SH%', ifnull(1 - (round(ifnull(ga.GA,0),2) / (round(ifnull(sa.SA,0),2))),1) as 'On_Ice_SV%', 
    ...
    as DZ_Faceoffs, ifnull(nz.NZ_Faceoffs,0) as NZ_Faceoffs
...
LEFT OUTER JOIN(SELECT p.game_id, p.name as name, p.pos as pos, COUNT(p.name) as CF, strength, period

    FROM (SELECT p.game_id, event_type, p1_team, p2_team, p.name as name, p.pos as pos, p.team as team, strength, period
        
        FROM (SELECT p.game_id, event_type, p1_team, p2_team, p.name, r.Team as team, p.pos, p.period, 
                case when (event_type = 'GOAL' or event_type='MISS' or event_type='SHOT') and p.p1_team = z.home_team 
                then home_strength || 'v' || away_strength 
            ...
            FROM (SELECT game_id, event_type, p1_team, p2_team, H1Name as name, H1Num as num, H1Pos as pos, period, 
                ...
                WHERE H1Pos != 'G'
                UNION ALL
                SELECT game_id, event_type, p1_team, p2_team, H2Name, H2Num, H2Pos, period, home_strength, away_strength
                FROM pbp
                WHERE H2Pos != 'G'
                UNION ALL
                SELECT game_id, event_type, p1_team, p2_team, H3Name, H3Num, H3Pos, period, home_strength, away_strength
                FROM pbp
                WHERE H3Pos != 'G'
                UNION ALL
                SELECT game_id, event_type, p1_team, p2_team, H4Name, H4Num, H4Pos, period, home_strength, away_strength
                FROM pbp
                WHERE H4Pos != 'G'
                UNION ALL
                SELECT game_id, event_type, p1_team, p2_team, H5Name, H5Num, H5Pos, period, home_strength, away_strength
                FROM pbp
                WHERE H5Pos != 'G'
                UNION ALL
                SELECT game_id, event_type, p1_team, p2_team, H6Name, H6Num, H6Pos, period, home_strength, away_strength
                FROM pbp
                WHERE H6Pos != 'G'
                UNION ALL
                
                SELECT game_id, event_type, p1_team, p2_team, A1Name, A1Num, A1Pos, period, home_strength, away_strength
                FROM pbp
                WHERE A1Pos != 'G'
                UNION ALL
                SELECT game_id, event_type, p1_team, p2_team, A2Name, A2Num, A2Pos, period, home_strength, away_strength
                FROM pbp
                WHERE A2Pos != 'G'
                UNION ALL
                SELECT game_id, event_type, p1_team, p2_team, A3Name, A3Num, A3Pos, period, home_strength, away_strength
                FROM pbp
                WHERE A3Pos != 'G'
                UNION ALL
                SELECT game_id, event_type, p1_team, p2_team, A4Name, A4Num, A4Pos, period, home_strength, away_strength
                FROM pbp
                WHERE A4Pos != 'G'
                UNION ALL
                SELECT game_id, event_type, p1_team, p2_team, A5Name, A5Num, A5Pos, period, home_strength, away_strength
                FROM pbp
                WHERE A5Pos != 'G'
                UNION ALL
                SELECT game_id, event_type, p1_team, p2_team, A6Name, A6Num, A6Pos, period, home_strength, away_strength
                FROM pbp
                WHERE A6Pos != 'G'
                ) as p
...
LEFT OUTER JOIN(SELECT p.name as name, p.pos as pos, COUNT(p.name) as CA, strength, period

    FROM (SELECT p.game_id, event_type, p1_team, p4_team, p.name as name, p.pos as pos, p.team as team, strength, period
        
        FROM (SELECT p.game_id, event_type, p1_team, p4_team, p.name, r.Team as team, p.pos, p.period, 
                case when (event_type = 'GOAL' or event_type='MISS' or event_type='SHOT') and p.p4_team = z.home_team 
                then home_strength || 'v' || away_strength 
            ...
            FROM
                ...
                WHERE H1Pos != 'G'
                UNION ALL
                SELECT game_id, event_type, p1_team, p4_team, H2Name, H2Num, H2Pos, period, home_strength, away_strength
                FROM pbp
                WHERE H2Pos != 'G'
                UNION ALL
                SELECT game_id, event_type, p1_team, p4_team, H3Name, H3Num, H3Pos, period, home_strength, away_strength
                FROM pbp
                WHERE H3Pos != 'G'
                UNION ALL
                SELECT game_id, event_type, p1_team, p4_team, H4Name, H4Num, H4Pos, period, home_strength, away_strength
                FROM pbp
                WHERE H4Pos != 'G'
                UNION ALL
                SELECT game_id, event_type, p1_team, p4_team, H5Name, H5Num, H5Pos, period, home_strength, away_strength
                FROM pbp
                WHERE H5Pos != 'G'
                UNION ALL
                SELECT game_id, event_type, p1_team, p4_team, H6Name, H6Num, H6Pos, period, home_strength, away_strength
                FROM pbp
                WHERE H6Pos != 'G'
                UNION ALL
                
                SELECT game_id, event_type, p1_team, p4_team, A1Name, A1Num, A1Pos, period, home_strength, away_strength
                FROM pbp
                WHERE A1Pos != 'G'
                UNION ALL
                SELECT game_id, event_type, p1_team, p4_team, A2Name, A2Num, A2Pos, period, home_strength, away_strength
                FROM pbp
                WHERE A2Pos != 'G'
                UNION ALL
                SELECT game_id, event_type, p1_team, p4_team, A3Name, A3Num, A3Pos, period, home_strength, away_strength
                FROM pbp
                WHERE A3Pos != 'G'
                UNION ALL
                SELECT game_id, event_type, p1_team, p4_team, A4Name, A4Num, A4Pos, period, home_strength, away_strength
                FROM pbp
                WHERE A4Pos != 'G'
                UNION ALL
                SELECT game_id, event_type, p1_team, p4_team, A5Name, A5Num, A5Pos, period, home_strength, away_strength
                FROM pbp
                WHERE A5Pos != 'G'
                UNION ALL
                SELECT game_id, event_type, p1_team, p4_team, A6Name, A6Num, A6Pos, period, home_strength, away_strength
                FROM pbp
                WHERE A6Pos != 'G'
                ) as p
...
LEFT OUTER JOIN(SELECT p.name as name, p.pos as pos, COUNT(p.name) as FF, strength, period

    FROM (SELECT p.game_id, event_type, p1_team, p.name as name, p.pos as pos, p.team as team, strength, period
...
LEFT OUTER JOIN(SELECT p.name as name, p.pos as pos, COUNT(p.name) as FA, strength, period

    FROM (SELECT p.game_id, event_type, p4_team, p.name as name, p.pos as pos, p.team as team, strength, period
...
LEFT OUTER JOIN(SELECT p.name as name, p.pos as pos, COUNT(p.name) as SF, strength, period
...
LEFT OUTER JOIN(SELECT p.name as name, p.pos as pos, COUNT(p.name) as SA, strength, period
...
LEFT OUTER JOIN(SELECT p.name as name, p.pos as pos, COUNT(p.name) as GF, strength, period
...
LEFT OUTER JOIN(SELECT p.name as name, p.pos as pos, COUNT(p.name) as GA, strength, period
...
LEFT OUTER JOIN(SELECT p.name as name, p.pos as pos, COUNT(p.name) as OZ_Faceoffs, strength, period
...
GROUP BY z.season, r.name, r.pos, strength.strength, strength.period;
'''
...
LEFT OUTER JOIN(SELECT p.game_id, p.name as name, p.pos as pos, SUM(p.CF) as CF, strength, period
...
LEFT OUTER JOIN(SELECT p.name as name, p.pos as pos, SUM(p.CA) as CA, strength, period
...
LEFT OUTER JOIN(SELECT p.name as name, p.pos as pos, SUM(p.FF) as FF, strength, period
...
LEFT OUTER JOIN(SELECT p.name as name, p.pos as pos, SUM(p.FA) as FA, strength, period
...
LEFT OUTER JOIN(SELECT p.name as name, p.pos as pos, SUM(p.SF) as SF, strength, period
...
LEFT OUTER JOIN(SELECT p.name as name, p.pos as pos, SUM(p.SA) as SA, strength, period
...
LEFT OUTER JOIN(SELECT p.name as name, p.pos as pos, SUM(p.GF) as GF, strength, period
...
LEFT OUTER JOIN(SELECT p.name as name, p.pos as pos, SUM(p.GA) as GA, strength, period
